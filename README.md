...
- `/` 登录页（3D 小狗背景 + 克制登录表单，支持 `?token=` 或 `#token=` 自动登录）
...
- 产品默认按 [`PRODUCT.md`](PRODUCT.md) 和 [`DESIGN.md`](DESIGN.md) 的 product register 执行，目标气质为“亲密、安静、可信”。
- 当前视觉基线是克制的私密日记型产品 UI：中性背景、清晰面板边界、紧凑阴影、玫瑰色只用于主操作 / 选中 / 重要状态。
- 前端避免大面积粉色渐变、装饰玻璃拟态、过圆卡片、厚阴影和纯装饰动效；动画只服务状态反馈、3D 登录背景和 todo 抽奖等功能场景。
