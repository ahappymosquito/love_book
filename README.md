...
- Todo 任务标签显示为“吃喝 / 玩乐 / 住宿 / 许愿”，`/todo` 主界面使用“今天想吃点 / 出去玩一玩 / 住一晚也好 / 悄悄许个愿”四个可折叠板块。底部新增输入先进入待确认队列，后端同步用 LLM 分类到 `food/play/stay/wish`，对非许愿项调用高德 MCP 搜索；单一结果显示待增加，多结果显示候选列表，用户确认后才创建正式 todo。工具栏刷新按钮仍可批量调用 LLM 更新未完成 todo 分类，已完成 todo 不参与刷新。
