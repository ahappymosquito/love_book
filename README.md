...
- 线下见面使用手动命名的 `meeting_sessions` 作为真实见面次数统计单位；一次周末或节假日见面可以归入同一个场次，场次内再保存双方提交的多条 `offline_meeting` 事件。双方都可整理自己或对方的事件归属，归入场次会自动把事件标为线下见面；场次开始/结束时间由场次内事件时间自动计算，不再手填日期。
...
app/
  ...
  api/
    ...
    routes/
      ...
      events.py           事件接口
      meeting_sessions.py 手动命名的线下见面场次接口，按归属事件自动返回时间范围
