...
- 线下见面使用内部 `meeting_sessions` 作为真实见面次数统计单位；创建或标记线下事件时自动建立同名见面，无需填写归属。一次见面可收纳双方多条 `offline_meeting` 事件，双方都能在见面界面点击标题改名、通过加号批量加入或移动已有记录；事件内容权限保持不变，见面时间范围由其中事件自动计算，空见面会自动清理。
...
app/
  ...
  api/
    ...
    routes/
      ...
      events.py           事件接口
      meeting_sessions.py 共享见面改名、批量归入和事件时间范围接口
...
- `/timeline` 事件列表首页（关系状态 hero、全部 / 见面分段、见面标题原地改名与批量添加记录、月份分组、狗狗空状态和底边栏导航）
- `/timeline/[id]` 事件详情（创建者可原地编辑标题、描述、发生时间和可见方式，也可标记线下见面；评论 / 语音 / 图片混排，底部输入栏支持文字、录音、相册）
