...
- Todo 任务标签显示为“吃喝 / 玩乐 / 住宿 / 许愿”，`/todo` 主界面使用“今天想吃点 / 出去玩一玩 / 住一晚也好 / 悄悄许个愿”四个可折叠板块。底部新增输入会立刻进入待确认队列并显示“正在解析”，后端同步用 LLM 分类到 `food/play/stay/wish`，对非许愿项调用高德 MCP 搜索；单一结果显示待增加，多结果显示候选列表，用户点击卡片查看详情后确认加入或丢弃，确认失败时保留卡片和错误提示。工具栏刷新按钮仍可批量调用 LLM 更新未完成 todo 分类，已完成 todo 不参与刷新。
