...
- 管理端 `/admin` 的 AI / 模型配置区先选择 OpenAI 或 Anthropic 协议，再编辑对应地址和 token；获取模型列表后在下拉框展示模型数量，选择模型会自动保存并真实调用一次 `food/play` 补全测试，也保留手动测试按钮。测试样例固定使用“江西小炒(西溪北苑东区店)”，先通过高德 MCP 获取真实 POI 证据，再让 LLM 基于 POI 名称、地址、类型判断分类，并在界面展示“高德取证 -> LLM 判断”过程。该测试用于发现高德失败、空回复、协议错配或模型不支持 chat/messages 的问题；分类补全默认给 64 tokens 输出预算，遇到长度耗尽且正文为空时用 256 tokens 重试一次；高德 `AMAP_MAPS_API_KEY` 单独罗列并可自定义保存。
