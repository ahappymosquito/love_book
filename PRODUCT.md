...
Meeting sessions remain an internal counting and grouping model rather than a visible form concept. Creating or marking an offline event automatically creates a same-title meeting, while the meeting view lets either partner click the meeting title to rename it and use the adjacent add action to collect more existing records. Event content permissions remain unchanged, event and meeting titles evolve independently after creation, empty meetings are cleaned up, and meeting date ranges are derived from assigned event times.
