...
# 3. 创建发布提交与 annotated tag
git add VERSION CHANGELOG.md web/package.json web/package-lock.json
git commit -m "chore: release 0.3.0"
git tag -a v0.3.0 -m "love_book 0.3.0"
```

只有明确推送 `vX.Y.Z` 标签才会发布 GHCR 镜像。发布工作流会拒绝与 `VERSION` 不一致的标签，并为前后端同时生成 `0.3.0` 与 `sha-xxxxxxx` 两类不可变标签。
...
```bash
LOVE_BOOK_VERSION=0.3.0 ./deploy_server.sh up
LOVE_BOOK_VERSION=0.3.0 ./deploy_server.sh status
...
mysqldump --single-transaction --routines --triggers \
  -h DB_HOST -u DB_USER -p DB_NAME > backups/love_book-before-0.3.0.sql

# named volume 媒体备份
docker run --rm \
  -v love_book_media:/source:ro \
  -v "$PWD/backups:/backup" \
  alpine:3.22 tar -czf /backup/love_book_media-before-0.3.0.tar.gz -C /source .
```
