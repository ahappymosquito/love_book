...
- Todo 任务标签前端显示为“吃喝 / 玩乐 / 住宿 / 许愿”；`/todo` 主界面分为“今天想吃点 / 出去玩一玩 / 住一晚也好 / 悄悄许个愿”四个板块，其中许愿板块默认展开。新增输入放在板块顶部，输入框右侧提供“新增”和“许愿”两个操作。“新增”必须创建本地“正在解析”待确认卡片，再由后端同步用 LLM 分类并调用高德 MCP 搜索；“许愿”必须直接创建 wish todo、展开许愿板块、打开详情并进入描述编辑，不进入 LLM / 高德待确认队列。当前用户保存了常用位置时，food/stay 等地点候选先用 5km 周边搜按距离优先，再用文本搜索补齐并去重，候选卡显示距离、地址、评分、人均等信息；待确认卡片默认展示 LLM 判断分类、高德候选信息和分类切换控件，用户可改到任意板块后再确认加入或丢弃。确认失败必须保留卡片并显示可重试错误，不能抛出未处理运行时错误；后端确认异常应回滚并返回明确 502，不删除候选。`/todo` 页面不再展示手动刷新分类按钮。
