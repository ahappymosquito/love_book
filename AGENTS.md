...
- 登录后的用户页面统一显示底边栏：`/timeline`、`/timeline/[id]`、`/create`、`/cycle`、`/todo`、`/me`；中间透明背景 Three.js 可爱加号模型按钮进入 `/create` 记一笔，模型直接露出不加红色圆形底，需要有明显尺寸、轻微上下浮动和持续旋转，桌面端底栏需要和主内容宽度平齐，`/me` 底栏入口显示设置图标，是当前用户资料和共享语录管理入口。
