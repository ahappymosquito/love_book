...
- 时间线事件支持 `event_kind`：默认 `memory`，手动选择或在详情页归入见面场次后会成为 `offline_meeting`；同 pair 双方都可以把自己或对方创建的事件整理进手动命名的 `meeting_sessions`，但不因此获得编辑对方标题、正文、发生时间、可见性或删除的权限。`/timeline` 的“见面”分段按场次统计真实见面次数，场次内再展示多条双方提交的线下见面事件；场次开始/结束时间由其中事件的 `occurred_at` 自动计算，缺失时回退 `created_at`，不再让用户手填日期。详情页修改所属场次必须点击“保存归属”后才生效，并显示保存状态；线下见面事件在“全部”事件列表中也需要柔和高亮显示。
