...
- 时间线事件支持 `event_kind`：默认 `memory`，手动选择或在详情页补标为 `offline_meeting` 后，可归入手动命名的见面场次；`/timeline` 的“见面”分段按 `meeting_sessions` 场次统计真实见面次数，场次内再展示多条双方提交的线下见面事件，线下见面事件在“全部”事件列表中也需要柔和高亮显示。
