...
- 登录后的用户页面统一显示底边栏：`/timeline`、`/timeline/[id]`、`/create`、`/cycle`、`/todo`、`/me`；中间 Three.js 动画按钮进入 `/create` 记一笔，`/me` 底栏入口显示设置图标，是当前用户资料和共享语录管理入口。
...
- `/me` 的“我的小档案”保留头像图片/emoji、用户名和邮箱编辑，用户名和邮箱通过保存按钮统一提交；下方折叠展示共享语录，再展示只读默认语录。
...
- 当前登录用户可在 `/me` 的“我的小档案”自助更新自己的用户名、邮箱、emoji 头像和头像图片；邮箱空字符串按 `NULL` 保存。
