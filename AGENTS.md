...
- Todo 任务标签前端显示为“吃喝 / 玩乐”；`/todo` 工具栏只保留一个刷新按钮，点击后批量调用 LLM 更新当前 pair 全部未完成 todo，已完成 todo 不显示刷新入口也不参与刷新；普通新建任务默认使用内部 `play` 分类，后续通过刷新标签更新。
...
- 管理端先选择 OpenAI 或 Anthropic，再编辑当前协议对应地址和 token；获取模型列表后需要在下拉框展示模型数量，选择模型后自动执行真实 `food/play` 补全测试，并保留手动测试连接按钮；测试必须能发现空回复、协议错配或模型不支持 chat/messages 的问题。分类补全默认给 64 tokens 输出预算，遇到长度耗尽且正文为空时用 256 tokens 重试一次。
