...
- 一言模块已弃用；普通日情话优先从当前 pair 的数据库语录库随机选取，没有自定义语录时从 `default_quotes` 全站共享兜底语录表随机选取。
- 节假日接口失败时必须静默降级，不能影响 `/timeline` 首页加载。
- `/quotes` 是登录后的情侣共享语录库接口；同一 pair 双方可添加、查看、删除共享语录，写接口必须在响应返回前完成数据库提交；前端编辑入口收在首页纪念日卡片右侧编辑按钮内。
