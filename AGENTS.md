...
- `/timeline` 首页顶部只展示关系状态、在一起天数、今日话语和必要纪念日/节日提醒；创建、周期、习惯等功能入口统一交给底边栏，顶部不展示这些跳转按钮，也不展示语录来源实现说明。
...
- Timeline 首页“今日话语”刷新不显示独立图标按钮；点击话语文案本身即可刷新，刷新中只使用轻微状态反馈，不把实现说明写到页面正文。
