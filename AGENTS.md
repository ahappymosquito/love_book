...
- 习惯页顶部是更高层次的液态玻璃月日期看板，每个日期格子按双方各一半展示；每个人的一半按当天启用习惯数量等分，完成一项填满一份对应颜色。双方当天全部启用习惯都完成时，日期格展示低调融合完成态并播放克制庆祝动画，低动效模式下只切换完成样式。
- 习惯页下方是上下排列的全宽液态玻璃折叠清单，桌面和移动端都不得改成左右两栏；默认展开自己、折叠对方；点击自己的习惯即可对当前选中日期完成/取消，支持点击过去日期补记。习惯颜色只在新增或进入编辑状态时选择，不在每条任务行常驻展示颜色选择器；完成后的任务行整体切换为鼠尾草绿色并播放完成反馈动画。
