...
- 登录后的用户页面统一显示底边栏：`/timeline`、`/timeline/[id]`、`/create`、`/cycle`、`/todo`、`/me`；底栏使用克制的 iOS 液态玻璃材质，普通导航项之间以共享玻璃选中块丝滑滑动，低动效模式下直接切换；进入 `/create` 时玻璃选中块淡出，只由中间透明背景 Three.js 按钮表达选中。创建模型使用中心对称三轴圆润加号和华丽球型笼，核心与球笼反向旋转并轻微上下浮动，直接露出且不加红色圆形底，低动效模式下静止；桌面端底栏需要和主内容宽度平齐，`/me` 底栏入口显示设置图标，是当前用户资料和共享语录管理入口。
