...
- Todo 任务标签前端显示为“吃喝 / 玩乐 / 住宿”；`/todo` 工具栏只保留一个刷新按钮，点击后批量调用 LLM 更新当前 pair 全部未完成 todo，已完成 todo 不显示刷新入口也不参与刷新；普通新建任务默认使用内部 `play` 分类，住宿视图新建使用内部 `stay` 分类，后续通过刷新标签更新。
- Todo 数据独立于时间线事件，不自动创建或更新 `/timeline` 事件。
- Todo 项目按 pair 双方共享，分为 `food` 吃喝、`play` 玩乐和 `stay` 住宿；默认玩乐项目为“唱歌、台球、看电影、拼乐高”。
