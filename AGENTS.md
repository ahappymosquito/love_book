...
- 登录后的用户页面统一显示底边栏：`/timeline`、`/timeline/[id]`、`/create`、`/cycle`、`/todo`、`/me`；中间 Three.js 可爱加号模型按钮进入 `/create` 记一笔，桌面端底栏需要和主内容宽度平齐，`/me` 底栏入口显示设置图标，是当前用户资料和共享语录管理入口。
...
- `/me` 的“我的小档案”保留头像图片/emoji、用户名和邮箱编辑；头像点击进入头像编辑，用户名/邮箱点击后在卡片内原地编辑，并通过保存按钮统一提交；下方折叠展示共享语录，用户语录和默认语录都使用清晰边界行，默认语录以只读行直接排在用户语录下面，不单独做说明板块。
