...
- 时间线事件支持 `event_kind`：默认 `memory`，创建或在详情页标记为 `offline_meeting` 时，后端必须自动建立以事件标题命名的 `meeting_sessions` 记录并原子归入；界面统一称为“见面”，不得再展示“见面场次”表单或说明。`/timeline` 的“见面”分段按内部 session 统计真实见面次数，同一次见面可收纳双方多条事件；双方都可点击见面标题原地改名，并通过标题旁加号批量加入或移动已有记录，但不因此获得编辑对方事件内容的权限。移动、取消标记或删除最后一条记录后需清理空见面；事件标题和见面标题后续独立维护。见面时间范围由事件 `occurred_at` 自动计算，缺失时回退 `created_at`。
- 事件详情顶部使用铅笔图标进入创建者专属的原地编辑，可修改标题、描述、发生时间和可见方式；日历爱心图标只负责标记或取消线下见面，对方仍只能参与共享见面的整理与改名。
