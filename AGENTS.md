...
- Todo 任务标签前端显示为“吃喝 / 玩乐 / 住宿 / 许愿”；`/todo` 主界面分为“今天想吃点 / 出去玩一玩 / 住一晚也好 / 悄悄许个愿”四个可折叠板块。底部新增输入会先创建本地“正在解析”待确认卡片，再由后端同步用 LLM 分类并对非许愿项调用高德 MCP 搜索；单一结果显示待增加，多结果显示候选列表，用户点击卡片查看详情后确认加入或丢弃，确认失败必须保留卡片并显示可重试错误，不能抛出未处理运行时错误。`/todo` 工具栏保留刷新按钮，可批量调用 LLM 更新当前 pair 全部未完成 todo，已完成 todo 不显示刷新入口也不参与刷新。
