...
- Login uses a two-layer mobile-first composition: a dedicated puppy hero stage in the upper portion, then a thumb-friendly scrapbook login panel anchored lower on the screen. Desktop keeps the puppy hero larger while the form remains the primary task surface. The puppy scene supports `hero` and `inline` variants, touch-safe click feedback, responsive camera framing, and reduced-motion soft/still fallbacks instead of one fixed desktop composition.
...
- Timeline home now starts with a tighter relationship hero band, a cleaner anniversary message card, lighter month-group containers, and a puppy-assisted empty state. Event lists reduce card nesting on mobile so scanning and tapping feel closer to a message timeline than a stack of heavy tiles.
