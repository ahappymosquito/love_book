...
- Timeline home starts with one compact relationship quote band that combines only the pair names, relationship day count, and current quote. The current quote text itself is the refresh target, so the band does not show a separate refresh icon button or a "今天的话" label, and it no longer shows anniversary or festival reminder pills in this top band. It does not duplicate bottom navigation destinations such as create, cycle, or habits in the top section, and it does not expose quote-source implementation labels. Below the quote band, a restrained segmented control switches between all memories and the offline-meeting view. Creating or marking an `offline_meeting` automatically creates a same-title meeting, so creation and detail surfaces never expose a “见面场次” form. The meeting view counts internal sessions as real meetings, lets either partner click a meeting title for inline rename, and places a compact add control beside it for batch-collecting existing records through a mobile-safe Sheet; session ranges remain derived from event times. Event creators edit title, description, occurred time, and visibility inline from a pencil action in the detail header, while the calendar-heart action only toggles meeting status. All-memory rows keep a soft meeting highlight and show the meeting name. Event lists reduce card nesting on mobile so scanning and tapping feel closer to a message timeline than a stack of heavy tiles. Visible page copy must speak from the user's point of view and must not describe implementation changes, layout optimizations, or release-note style updates.
