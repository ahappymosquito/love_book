...
- `/habits` is a daily check-in dashboard: a large month board shows both partners' completion in each date cell, with each partner owning half the cell and each active habit taking an equal segment. The lower panels are collapsible, with the current user's panel open by default and the counterpart panel read-only. Habit colors stay low-saturation rose, peach, sage, and adjacent warm/mint swatches; full pair completion may use a brief fused-color celebration that respects reduced motion.
