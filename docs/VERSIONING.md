...
镜像发布成功后，用命名 SSH 主机把稳定版送到生产，不要手写 IP 或把私钥放进仓库。智能体只读 `LOVE_BOOK_SSH_HOSTS`（`~/.ssh/config` 的 Host 名）；CLI 是 `python scripts/deploy_host.py`。第一项做检查，`User=root` 的项跑更新器。完整说明见 [`DEPLOYMENT.md`](DEPLOYMENT.md)。
