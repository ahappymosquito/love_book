...
Love Book is used by a fixed pair of partners who enter through private token links or their own short login name and security password, then return from phones and desktops to record daily memories, arrange todo plans, track personal habits together, check cycle notes, and manage their shared account setup. Before login, visitors can play a public code-drawn meadow-gathering runner and compare scores on a global Top 3. Admin users configure pairs, entry links, contact details, AI model settings, and operational records.
...
The unauthenticated surface is a game-first Canvas 2D meadow named 花田拾光 rather than a decorative 3D login scene. Xiaohua—the gold-and-white puppy with a forehead blaze, flower garland, and green pendant—is drawn every frame from ellipses and curves driven by a pose function. Hills, grass, flowers, rocks, stumps, logs, brambles, and birds are generated the same way, with no sprite atlas. Visitors without a session token see a living title meadow and a start menu first; play begins only from 开始游戏. A stored Bearer token or `?token=` / `#token=` entry link skips the game and enters `/timeline`. Xiaohua runs through four light chapters—dawn meadow, noon breeze, dusk path, and starry return—while jumping, double-jumping after noon, crouching, gathering petals and letters, spending two starting hearts on collisions, and celebrating records. The runner fills the complete viewport; scenery is hashed and scrolled to the available shape instead of letterboxing. Desktop play uses left click or Up/Space to jump, with hold height on those fine pointers, and held right click or Down/S to crouch. Mobile uses two invisible zones: the left half crouches while held and the right half jumps a full arc on press. Instructions stay behind a corner help button. Obstacle drawings use the same world metrics as their hitboxes, with a compact contact shadow. Difficulty still follows speed-gated Chrome-runner principles: ground obstacles lead, low birds join after 18 seconds, fair two-action sequences begin after 40 seconds, continuous acceleration tightens reaction time, randomized width-aware gaps prevent a metronomic rhythm, and duplicate history avoids repetitive runs. Score keeps a distance baseline and adds pickup points, combo scaling, and near-miss sparks. A hit spends one heart and grants brief invincibility; the last heart ends the run and shows the chapter, petals, and best combo. Login, leaderboard, help, and name overlays pause and lock runner input. The public leaderboard retains only the global Top 3 and lets qualifying players leave a short name.
...
Avoid cold neutral dashboards, lifeless gray panels, harsh saturated colors, and jittery motion. Also avoid the AI pink glass-card look: blurred panels, generic purple-blue gradients, gradient text, low-contrast pastel copy, random decoration, and childish sticker overload.
...
- Use smooth motion for feedback and celebratory moments only; do not delay core tasks or create visual noise.

## Accessibility & Inclusion

Maintain at least the current accessibility level and fix visible issues while redesigning. Priorities are readable contrast, visible focus states, 44px minimum touch targets, mobile-safe layout, reduced-motion support, and standard controls for forms, dialogs, sheets, and buttons.
