...
Love Book is used by a fixed pair of partners who enter through private token links or their own short login name and security password, then return from phones and desktops to record daily memories, arrange todo plans, track personal habits together, check cycle notes, and manage their shared account setup. Before login, visitors can play a public pixel puppy runner and compare scores on a global Top 3. Admin users configure pairs, entry links, contact details, AI model settings, and operational records.
...
The unauthenticated surface is a game-first detailed pixel-storybook prairie rather than a decorative 3D login scene. Pixel Xiaohua runs, jumps, crouches, stumbles, and celebrates records while the login form stays available without dominating play. The runner fills the complete viewport width and height; scenery is cropped and tiled to the available shape instead of letterboxing inside a centered ratio frame. The neutral black-and-white login overlay is collapsed by default on every orientation. Desktop play uses left click or Up/Space to jump and held right click or Down/S to crouch. Mobile starts on the first grass interaction: holding the left half crouches until release, while tapping the right half jumps. Instructions stay behind a corner help button. Obstacles crop away atlas transparency so their visible silhouettes match their collision scale, then receive a crisp dark pixel outline and grounded contact shadow for quick recognition. Difficulty follows speed-gated Chrome-runner principles: ground obstacles lead, low birds join after 18 seconds, fair two-action sequences begin after 40 seconds, continuous acceleration tightens reaction time, randomized width-aware gaps prevent a metronomic rhythm, and duplicate history avoids repetitive runs. Login, leaderboard, help, and name overlays pause and lock runner input. The public leaderboard retains only the global Top 3 and lets qualifying players leave a short name.
