...
- 未登录首页提供像素「花田拾光」与匿名全站 Top 3 排行榜；分数含路程、拾光和连击加成，达到入榜门槛后可留下 1–12 字符的 Unicode 名字。
...
- `/` 登录页（全视口像素「花田拾光」、桌面按住跳得更高 / 移动端左侧按住下蹲与右侧点按跳跃、晨昏光景、拾光计分、小心心、匿名 Top 3 排行榜和默认收起的中性登录卡，支持安全密码、手动 token，以及 `?token=` / `#token=` 自动登录）
...
- 前端避免晃眼高饱和色、清冷灰后台、装饰玻璃拟态、渐变文字和低对比彩色文本。登录页用 Canvas 2D「花田拾光」承担陪伴感，Timeline 首页不展示动态小狗，以实体语录内容层和静态空状态保持阅读安静；Three.js 仅继续服务 Todo 抽奖。
