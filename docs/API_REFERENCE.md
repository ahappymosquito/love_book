...
- 未登录首页提供代码绘制「花田拾光」与匿名全站 Top 3 排行榜；无 token 先看到开始菜单，有 token 直接进入手账。分数含路程、拾光和连击加成，达到入榜门槛后可留下 1–12 字符的 Unicode 名字。
...
.env.example                  含 LOVE_BOOK_SSH_HOSTS
VERSION                       前后端唯一应用版本
...
- `/` 登录页（无 token 时全视口代码绘制「花田拾光」加开始弹窗；桌面按住跳得更高 / 移动端左侧按住下蹲与右侧点按跳跃；晨昏光景、拾光计分、小心心、匿名 Top 3；登录在菜单内；支持安全密码、手动 token，以及 `?token=` / `#token=` 自动进入 `/timeline`）
...
- 前端避免晃眼高饱和色、清冷灰后台、装饰玻璃拟态、渐变文字和低对比彩色文本。登录页用代码绘制的 Canvas 2D「花田拾光」承担陪伴感，Timeline 首页不展示动态小狗，以实体语录内容层和静态空状态保持阅读安静；Three.js 仅继续服务 Todo 抽奖。
