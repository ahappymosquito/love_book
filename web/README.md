...
| `/admin` | 管理控制台：先用 admin key 验证，再创建配对、选择 token 有效期、复制 token / 入口链接；Clipboard API 失败时自动降级复制；AI 配置会保存上次模型列表和选中模型，测试时先用高德 MCP 获取“江西小炒(西溪北苑东区店)”POI 证据，再展示高德类型判断和 LLM 诊断 |
| `/timeline` | 当前用户的事件流，含纪念日、语录、todo / 周期入口和月份分组 |
| `/timeline/[id]` | 事件详情，合并的内容流（评论 / 语音气泡 / 图片缩略），底部输入栏支持文字 / 按住说话 / 图片 |
| `/create` | 新建事件 |
| `/todo` | 双人共享 To Do 式工作区，默认展示全部未完成事项，按要完成时间排序，含左侧列表导航、任务列表、完成折叠区、详情内日期安排、工具栏一键 AI 刷新未完成标签、评论作者、照片折叠、餐厅搜索、随机抽奖和打卡详情 |
