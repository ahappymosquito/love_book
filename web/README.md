...
| 路由 | 说明 |
| --- | --- |
| `/` | 精细像素绘本风小花跑酷登录页：全视口画幅、鼠标与触控适配、全站 Top 3、安全密码与 token 登录，支持 `?token=` 或 `#token=` 自动登录 |
...
- **登录主路径**：`/` 使用以游戏为主的全视口 Canvas 2D 小花跑酷，不保留居中比例框；角色、速度、跳跃和障碍使用设备无关逻辑单位。障碍间距按速度、宽度与随机系数生成，并抑制连续重复。电脑端鼠标左键或 `↑/Space` 跳跃、右键或 `↓/S` 按住趴下；手机左半区按住趴下、右半区点按跳跃，首次操作即开跑，角落 `?` 展示玩法。障碍素材按真实透明边界裁切，并通过像素轮廓和接触阴影保持可见大小与碰撞范围直观一致。中性登录卡默认收起，登录卡、排行榜、玩法和留名浮层打开时会暂停并锁住游戏输入，管理员入口和 token 自动登录逻辑保持原样。
