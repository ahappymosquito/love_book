...
| `/timeline` | 当前用户的事件流首页，含关系话语板块、纪念日提醒、月份分组和狗狗空状态创建入口 |
...
- **Timeline 首页**：`/timeline` 顶部为单一关系话语板块，合并关系状态、在一起天数、今日话语和纪念日提醒，不放创建、周期、习惯等底边栏已有入口；手机端事件项保持更轻的行式卡片，空状态使用 inline 小狗和现有创建弹层联动。
...
- `/timeline` 顶端关系话语板块展示双方昵称、在一起第 N 天、520/1314/整月纪念、固定恋爱节日、中国大陆节假日/调休标签和最终文案。
- 一言模块已弃用；普通日文案由后端从当前 pair 的数据库语录库和 `default_quotes` 全站共享兜底语录表合并后的随机池中选取。
- `/timeline` 顶部只保留刷新今日话语的图标按钮，不展示“本地语录兜底”等实现来源说明；共享语录添加、查看和删除集中在 `/me`，提醒接口失败时仍会用 `me.love_started_on` 做基础天数兜底。
## 周期日历 Dashboard
