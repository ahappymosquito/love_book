...
| `/create` | 新建事件 |
| `/todo` | 双人共享 To Do 式工作区，首屏是“要一起做的事情”，按“今天想吃点 / 出去玩一玩 / 住一晚也好 / 悄悄许个愿”四个默认折叠板块展示；新增输入放在板块顶部，提交后立刻进入待确认队列并显示解析中，解析后点击卡片查看待增加详情或高德候选列表，再确认加入或丢弃；确认失败会留在详情里展示错误；正式详情内使用单一日期安排、评论作者、照片折叠和高德取证信息；移动端详情操作区避让底边栏，日期保存后在任务行右侧用状态色 badge 展示 |
