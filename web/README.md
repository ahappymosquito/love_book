...
- **登录主路径**：`/` 使用以游戏为主的全视口 Canvas 2D 小花跑酷，不保留居中比例框；角色、速度、跳跃和障碍使用设备无关逻辑单位。障碍间距按速度、宽度与随机系数生成，并抑制连续重复。电脑端鼠标左键跳跃、右键按住趴下；手机首次轻点即开跑，轻点跳跃、长按趴下，角落 `?` 展示玩法。中性登录卡默认收起，登录卡、排行榜、玩法和留名浮层打开时会暂停并锁住游戏输入，管理员入口和 token 自动登录逻辑保持原样。
