...
| `/timeline` | 当前用户的事件流首页，含关系话语板块、全部 / 见面分段、月份分组、可点击改名并批量添加记录的线下见面时间河流和狗狗空状态创建入口 |
| `/timeline/[id]` | 事件详情，创建者可从顶部铅笔原地编辑完整事件信息、用日历爱心标记或取消线下见面；合并的评论 / 语音 / 图片内容流和底部输入栏覆盖全局底边栏 |
...
- **Timeline 首页**：`/timeline` 顶部为单一关系话语板块，只合并关系状态、在一起天数和当前话语，不放创建、周期、习惯等底边栏已有入口；下方使用“全部 / 见面”分段，线下见面由 `event_kind=offline_meeting` 标记并自动建立同名见面，在全部列表中柔和高亮。见面分段按真实见面组织时间河流，标题点击后原地改名，旁边加号通过移动端安全的 Sheet 批量加入或移动已有记录；手机端事件项保持更轻的行式卡片，空状态使用 inline 小狗和现有创建弹层联动。
