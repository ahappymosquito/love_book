...
- 一言模块已弃用；普通日文案由后端优先取当前 pair 的数据库语录库，没有自定义语录时使用 `default_quotes` 全站共享兜底语录表。
- `/timeline` 在纪念日卡片右侧提供语录编辑图标，可添加、查看和删除当前 pair 的共享语录；保存后编辑区自动收起，提醒接口失败时仍会用 `me.love_started_on` 做基础天数兜底。
## 周期日历 Dashboard
