...
| 路由 | 说明 |
| --- | --- |
| `/` | 代码绘制「花田拾光」登录页：无 token 时全视口花田加开始弹窗，有 token 直接进入手账；晨昏光景、拾光与小心心、鼠标与触控适配、全站 Top 3、安全密码与 token 登录 |
...
- **登录主路径**：`/` 在无 token 时使用全视口 Canvas 2D「花田拾光」，小狗、障碍和花田全部代码绘制，不加载图集；有本地 token 或 `?token=` / `#token=` 时不挂游戏，直接进入 `/timeline`。未登录先弹出开始菜单，点「开始游戏」才开跑。角色、速度、跳跃和障碍使用设备无关逻辑单位。跑酷按晨光 / 午后 / 黄昏 / 星夜换光景，分数由路程、花瓣信封和连击组成，开局两颗小心心，撞到障碍先失去一颗再短暂无敌。电脑端鼠标左键或 `↑/Space` 跳跃且按住跳得更高、右键或 `↓/S` 按住趴下，午后可二段跳；手机左半区按住趴下、右半区点按完整跳跃，角落 `?` 展示玩法。障碍间距按速度、宽度与随机系数生成，并抑制连续重复。登录、排行榜、玩法和留名弹窗打开时会暂停并锁住游戏输入，管理员入口保持原样。
