...
| 路由 | 说明 |
| --- | --- |
| `/` | 精细像素绘本风小花跑酷登录页：居中比例画幅、左右触控、全站 Top 10、安全密码与 token 登录，支持 `?token=` 或 `#token=` 自动登录 |
...
- **登录主路径**：`/` 使用以游戏为主的 Canvas 2D 小花跑酷。桌面/横屏将 16:9 画幅居中并限制到 1440×810，竖屏使用带 8px 安全边距的居中 3:4 画幅；角色、速度、跳跃和障碍使用设备无关逻辑单位。手机左半画幅按住趴下、右半点按跳跃，键盘对应 `↓/S` 与 `Space/↑`。登录卡、排行榜和留名弹窗始终位于触控层上方，管理员入口和 token 自动登录逻辑保持原样。
